...
public class BankAccount {
    ...
    public BankAccount(String username, String password, String[] accountBalances, String[] latestTransactions){
        ...
        // If there isn't a transaction at any given position, the log is filled with null.
        for (int i = 0; i < latestTransactions.length; i++)
            if (latestTransactions[i].equals("null"))
                latestTransactions[i] = null;
        transactionLog = latestTransactions;

        // Initializes all account types.
        // If there is no balance on an account type, the account is initialized at an inactive state.
        int accountTypeID = 0;
        for (String balanceStr : accountBalances){
            try {
                accounts.add(new Account(accountTypeID, Double.parseDouble(balanceStr))); // Active
            } catch (Exception e){
                accounts.add(new Account(accountTypeID)); // Inactive
            }
            accountTypeID++;
    ...
    public String withdraw(Account account, double requestedWithdrawal){
        ...
        if (finalWithdrawalAmount < requestedWithdrawal)
            return "Withdrew $" + finalWithdrawalAmount + " due to insufficient funds.";
    ...
    public String transfer(Account fromAccount, Account toAccount, double requestedTransfer){
        ...
        if (finalTransferAmount < requestedTransfer)
            return "Transferred $" + finalTransferAmount + " due to insufficient funds.";
    ...
    public void addBankAccountToDatabase(){
        try {
            ...
            System.out.println("[System Error] Unable to save bank account " + username + ".");
        }
    }
    ...
    public void updateBankAccountDatabase(){
        ...
        try {
            ...
            System.out.println("[System Error] Unable to update account " + username + ".");
            return;
        }
    ...
    public static void loadDatabase() {
        ...
        try {
            ...
            BufferedReader reader = new BufferedReader(new FileReader("BankAccounts.txt"));
            long accountID = -1; // Tracks the current line position in the file, used for logging bank accounts whose data is corrupted.
